...
class ASTModel(nn.Module):
    ...
    def frequency_attention(self, x):
        ...
        B, N, C = x.shape
        f_dim = self.f_dim  # 미리 계산된 f_dim 사용
        t_dim = N // f_dim

        # 주파수 방향으로 어텐션 계산
        x = x.transpose(1, 2).view(B, C, f_dim, t_dim)
        freq_attn = nn.functional.softmax(self.freq_attn_weights, dim=1)
        x = (x * freq_attn.unsqueeze(-1)).view(B, C, N).transpose(1, 2)
